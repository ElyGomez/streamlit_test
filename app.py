"""Aplicación Streamlit para simular el lanzamiento de una moneda."""
...
def toss_coin(n):
    """Simula el lanzamiento de una moneda n veces y muestra la media acumulada."""
...
if start_button:
    ...
    result_mean = toss_coin(number_of_trials)
    st.session_state['df_experiment_results'] = pd.concat([
        st.session_state['df_experiment_results'],
        pd.DataFrame(
            data=[[st.session_state['experiment_no'], number_of_trials, result_mean]],
            columns=['no', 'iteraciones', 'media']
        )
    ], axis=0)
